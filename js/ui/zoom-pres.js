...
(function () {
  ...
  function openExternalPresentation() {
    ...
    var slidesForExt = sl.map(function (s) {
      var rawPct = (s.imageUrl && s.innerSize && s.innerSize.widthPct != null) ? s.innerSize.widthPct : 45;
      var textPct = s.imageUrl ? Math.max(10, Math.min(90, rawPct)) : 45;
      return {
        title: s.title,
    ...
    var html = '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"/><title>발표 — ScholarSlide AI</title><style>'
      ...
      + '<button class="pb" onclick="toggleNotes()">📝 노트</button><div class="sep"></div><button class="pb" id="ext-sync-btn" onclick="toggleSync()" title="내부 창과 슬라이드 번호 동기화">' + (syncOn ? '🔗 동기화 ON' : '🔗 동기화 OFF') + '</button><button class="pb" onclick="window.close()">✕ 닫기</button></div><div class="notes" id="notes"></div>'
