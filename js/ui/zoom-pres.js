...
(function () {
  ...
  window._setSlideTitleFontScale = function (v) { _slideTitleFontScale = v; };
  window.getSlideTitleFontScale = function () { return _slideTitleFontScale; };
  ...
  function changeSlideTitleFontScale(delta) {
    _slideTitleFontScale = Math.max(30, Math.min(300, _slideTitleFontScale + delta));
    applySlideFontScale();
  }
  ...
  function applySlideFontScale() {
    ...
    var presTitleLbl = document.getElementById('pres-title-font-val');
    var slideTitleLbl = document.getElementById('slide-title-font-val');
    if (canvas) {
      canvas.style.setProperty('--slide-font-scale', _slideFontScale / 100);
      canvas.style.setProperty('--slide-title-font-scale', _slideTitleFontScale / 100);
    }
    if (lbl) lbl.textContent = _slideFontScale + '%';
    if (presLbl) presLbl.textContent = _slideFontScale + '%';
    if (presTitleLbl) presTitleLbl.textContent = _slideTitleFontScale + '%';
    if (slideTitleLbl) slideTitleLbl.textContent = _slideTitleFontScale + '%';
  }
  ...
  function changePresTitleFontScale(delta) {
    _slideTitleFontScale = Math.max(30, Math.min(300, _slideTitleFontScale + delta));
    applySlideFontScale();
    if (typeof window.renderPresSlide === 'function') window.renderPresSlide();
  }
  ...
  function urlToDataUrl(url) {
    var u = String(url || '');
    if (u.indexOf('data:') === 0) return Promise.resolve(u);
  ...
  function openExternalPresentation() {
    ...
    var getSlideImageItems = function (s) {
      var arr = [];
      var imgs = (typeof window.getSlideImages === 'function' ? window.getSlideImages(s) : null) || (Array.isArray(s.images) && s.images.length ? s.images : []);
      if (!imgs.length && (s.imageUrl || s.imageUrl2)) {
        if (s.imageUrl) arr.push({ url: s.imageUrl, slideImage: s.slideImage1 || null });
        if (s.imageUrl2) arr.push({ url: s.imageUrl2, slideImage: s.slideImage2 || null });
      } else {
        for (var i = 0; i < imgs.length; i++) if (imgs[i] && imgs[i].url) arr.push({ url: imgs[i].url, slideImage: (imgs[i].slideImage !== undefined ? imgs[i].slideImage : null) });
      }
    ...
    var itemsToConvert = [];
    slidesForExt.forEach(function (s) {
      if (s.imageItems) s.imageItems.forEach(function (it) {
        var u = String(it.url || '');
        if (u && u.indexOf('data:') !== 0) itemsToConvert.push(it);
      });
    });
    var convertAndOpen = function () {
      if (itemsToConvert.length === 0) { doOpen(slidesForExt); return; }
      if (typeof window.showToast === 'function') window.showToast('🖼 이미지 변환 중...');
      Promise.all(itemsToConvert.map(function (it) {
        return urlToDataUrl(it.url).then(function (dataUrl) { it.url = dataUrl || it.url; });
      })).then(function () {
        slidesForExt.forEach(function (s) {
          if (s.imageItems) { s.imageUrls = s.imageItems.map(function (it) { return it.url; }); s.imageUrl = s.imageItems[0] ? s.imageItems[0].url : null; }
        });
        doOpen(slidesForExt);
      }).catch(function () {
        if (typeof window.showToast === 'function') window.showToast('⚠️ 이미지 변환 실패 — 텍스트만 표시됩니다');
        doOpen(slidesForExt);
      });
    };
    var doOpen = function (slidesForExt) {
      ...
      var slidesJSON = JSON.stringify(slidesForExt).replace(/<\/script>/gi, '<\\/script>');
      var html = '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"/><title>발표 — ScholarSlide AI</title><style>'
        ...
        + 'function escU(u){return String(u||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;");}'
      ...
      try {
        var w = window.open('', '_blank', 'width=1280,height=800,resizable=yes,scrollbars=no');
        if (!w) {
  ...
  window.changeSlideTitleFontScale = changeSlideTitleFontScale;
  window.applySlideFontScale = applySlideFontScale;
  window.changePresFontScale = changePresFontScale;
  window.changePresTitleFontScale = changePresTitleFontScale;
