...
+ 'var _viewerMaximized = false; var _viewerRestoreRect = { x: 100, y: 100, w: 900, h: 700 };'
+ 'function toggleViewerFullscreen() { if (_viewerMaximized) { try { window.moveTo(_viewerRestoreRect.x, _viewerRestoreRect.y); window.resizeTo(_viewerRestoreRect.w, _viewerRestoreRect.h); } catch(e){} _viewerMaximized = false; document.removeEventListener("keydown", _viewerFsEsc); } else { try { _viewerRestoreRect = { x: window.screenX, y: window.screenY, w: window.outerWidth, h: window.outerHeight }; window.moveTo(screen.availLeft, screen.availTop); window.resizeTo(screen.availWidth, screen.availHeight); } catch(e){} _viewerMaximized = true; document.addEventListener("keydown", _viewerFsEsc); } }'
+ 'function _viewerFsEsc(e) { if (e.key === "Escape" && _viewerMaximized) { toggleViewerFullscreen(); } }'
