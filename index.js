...
async function generateImage(prompt) {
  // ⑨ Use dedicated image API key input if set, otherwise fall back to active key
  let key; try { key = getImageApiKey(); } catch { return null; }
  // Attempt 1: gemini-2.0-flash-exp (image generation, more stable endpoint)
  try {
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${key}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        contents: [{ parts: [{ text: `Create a professional academic diagram for a presentation slide: ${prompt}. Clean minimal style, blue-grey tones.` }] }],
  ...
  } catch (e) { console.warn('[gemini-2.0-flash-exp]', e.message); }
  // Attempt 2: gemini-2.5-flash-preview-image-generation
  try {
    const res2 = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-image-generation:generateContent?key=${key}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: `Create a professional academic diagram for: ${prompt}` }] }], generationConfig: { responseModalities: ['TEXT', 'IMAGE'] } })
    });
    if (res2.ok) { const data2 = await res2.json(); const imgPart = data2.candidates?.[0]?.content?.parts?.find(p => p.inlineData); if (imgPart?.inlineData?.data) return `data:${imgPart.inlineData.mimeType || 'image/png'};base64,${imgPart.inlineData.data}`; }
  } catch (e2) { console.warn('[gemini-2.5-flash-img]', e2.message); }
  // Attempt 3: imagen-4 (requires allowlist)
  try {
  ...
  } catch (e3) { console.warn('[imagen-4]', e3.message); }
  return null;
}
...
async function handleFileUpload(e) {
  ...
  fileName = file.name; showLoading('파일에서 텍스트 추출 중...', file.name, 20);
  try {
    ensurePDFWorker(); let text = '';
    if (file.type === 'application/pdf') {
      const buf = await file.arrayBuffer();
      window._pdfArrayBuffer = buf.slice(0); // store for new window
      // ─ Launch PDF preview (pass a copy of the buffer) ─
      loadPdfPreview(buf.slice(0), file.name);
    ...
    renderLeftPanel(); enableMainBtns();
  } catch (err) { console.error(err); showToast('❌ 파일 처리 실패: ' + err.message); }
  finally { hideLoading(); if (e.target) e.target.value = ''; }
}

function loadFromTextInput() {
  const val = document.getElementById('text-paste-input')?.value?.trim();
  if (!val) { showToast('⚠️ 텍스트를 입력하세요'); return; }
  rawText = val; fileName = '직접입력.txt';
  showToast('✅ 텍스트 로드 완료'); renderLeftPanel(); enableMainBtns();
}

function enableMainBtns() {
  ['fetch-sources-btn', 'save-session-btn'].forEach(id => { const el = document.getElementById(id); if (el) el.disabled = false; });
}

function saveContent(type) {
  let content = '', ext = 'txt';
  if (type === 'raw') { content = rawText; ext = 'txt'; }
  else if (type === 'summary') { content = summaryText; ext = 'md'; }
  else if (type === 'script') { content = presentationScript.map((s, i) => `## 슬라이드 ${i + 1}: ${slides[i]?.title || ''}\n\n${s}`).join('\n\n---\n\n'); ext = 'md'; }
  if (!content) { showToast('⚠️ 저장할 내용이 없습니다'); return; }
  const blob = new Blob([content], { type: 'text/plain' });
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `scholarslide_${type}.${ext}`; a.click();
  showToast(`✅ ${type} 저장됨`);
}
...
function _bgBarUpdate() {
  let bar = document.getElementById('bg-job-bar');
  if (!bar) return;
  if (!_bgJob.running) {
  ...
  const pct = _bgJob.total > 0 ? Math.round((_bgJob.done / _bgJob.total) * 100) : 0;
  bar.innerHTML = `
    <span style="font-size:11px;color:var(--accent)">🎨 이미지 생성 중</span>
    <span style="font-size:10px;color:var(--text2);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin:0 8px">${escapeHtml(_bgJob.label)}</span>
  ...
  `;
  // 전역 진행률도 함께 업데이트
  showGlobalProgress(`🎨 이미지 생성 중 (${_bgJob.done}/${_bgJob.total})`, pct, '🎨');
}
let _bgJobCancelled = false;
function _cancelBgJob() { _bgJobCancelled = true; _bgJobEnd('⏹ 이미지 생성 중단됨'); }

// Image generation state (for API key from input)
let _imageApiKey = ''; // overrides _activeApiKey for image generation if set

// Slide zoom state (보기 확대·축소, 50%~200%)
let _slideZoom = 100; // percent

// 슬라이드 내부 폰트 크기 (제목·불릿만, 30%~300%)
let _slideFontScale = 100; // percent
// storage.js applyWorkspaceSnapshot에서 복원 시 호출
...
function applySlideZoom() {
  ...
  const lbl = document.getElementById('slide-zoom-val');
  const scale = _slideZoom / 100;
  if (canvas) {
    canvas.style.zoom = _slideZoom + '%';
    // Firefox 등 zoom 미지원 브라우저용 fallback: transform으로 동일 효과
    if (!supportsZoom()) {
      canvas.style.zoom = '';
...
function applySlideFontScale() {
  const canvas = document.getElementById('slides-canvas');
  const lbl = document.getElementById('slide-font-val');
  const presLbl = document.getElementById('pres-font-val');
...
function openExternalPresentation() {
  ...
  const w = window.open('', '_blank', 'width=1280,height=800,resizable=yes,scrollbars=no');
  if (!w) { showToast('⚠️ 팝업이 차단됨 — 팝업 허용 후 다시 시도하세요'); return; }

  // 발표/외부발표에서도 마크다운·HTML 렌더링 적용을 위해 미리 렌더된 HTML 전달
  const slidesForExt = slides.map(s => ({
    ...s,
    titleHtml: markdownToHtml(s.title),
    bulletsHtml: (s.bullets || []).map(b => markdownToHtml(b))
  }));
  const slidesJSON = JSON.stringify(slidesForExt);
  const scriptJSON = JSON.stringify(presentationScript);
  const styleMode = slideStyle;
  const startIdx = activeSlideIndex;
  const fontScale = _slideFontScale / 100;

  const html = `<!DOCTYPE html><html lang="ko"><head>
<meta charset="UTF-8"/>
<title>발표 — ScholarSlide AI</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{background:#111;display:flex;flex-direction:column;height:100vh;overflow:hidden;font-family:'DM Sans',sans-serif,system-ui}
...
.pb{background:rgba(79,142,247,0.18);border:1px solid rgba(79,142,247,0.35);color:#9ecbff;border-radius:8px;padding:5px 13px;font-size:12px;cursor:pointer;white-space:nowrap}
.pb:hover{background:rgba(79,142,247,0.35)}
.pb.sm{padding:5px 9px;font-size:13px;font-weight:700;min-width:30px}
.counter{font-size:13px;color:#9ecbff;min-width:56px;text-align:center}
.zv{font-size:10px;color:rgba(158,203,255,0.65);min-width:36px;text-align:center}
.sep{width:1px;height:18px;background:rgba(79,142,247,0.2);flex-shrink:0}
.notes{background:rgba(12,16,24,0.97);color:#a0aab8;font-size:12px;padding:7px 18px;line-height:1.6;display:none;border-top:1px solid #1e2332;max-height:110px;overflow-y:auto}
.notes.show{display:block}
</style>
</head><body>
<div class="pres-area" id="pres-area">
  <div class="pres-inner" id="pres-inner"></div>
</div>
<div class="pres-controls">
  <button class="pb" onclick="nav(-1)">◀ 이전</button>
  <span class="counter" id="counter">1/1</span>
  <button class="pb" onclick="nav(1)">다음 ▶</button>
  <div class="sep"></div>
  <button class="pb sm" onclick="chZoom(-10)" title="축소 (− 또는 Ctrl+9)">−</button>
  <span class="zv" id="zv">100%</span>
  <button class="pb sm" onclick="chZoom(+10)" title="확대 (+ 또는 Ctrl+0)">+</button>
  <button class="pb" onclick="fitZoom()" title="화면에 맞추기 (F)">⊡ 맞추기</button>
  <div class="sep"></div>
  <button class="pb sm" onclick="chFontScale(-0.1)" title="폰트 줄이기">A−</button>
  <span class="zv" id="fv">100%</span>
  <button class="pb sm" onclick="chFontScale(0.1)" title="폰트 키우기">A+</button>
  <div class="sep"></div>
  <button class="pb" onclick="toggleNotes()">📝 노트</button>
  <button class="pb" onclick="window.close()">✕ 닫기</button>
</div>
...
function applyZoom(){
  var el=document.getElementById('pres-inner');
...
function chFontScale(d){
  fontScale=Math.max(0.3,Math.min(3,fontScale+d));
  var fv=document.getElementById('fv');
  if(fv) fv.textContent=Math.round(fontScale*100)+'%';
  render();
}
function fitZoom(){
  var area=document.getElementById('pres-area');
  var inner=document.getElementById('pres-inner');
  if(!area||!inner) return;
  var aw=area.clientWidth-32, ah=area.clientHeight-32;
  var iw=inner.offsetWidth||900, ih=inner.offsetHeight||506;
  if(!iw||!ih) return;
  zoom=Math.round(Math.max(25,Math.min(200,Math.min(aw/iw,ah/ih)*100)));
  applyZoom();
}

function render(){
  var s=SL[idx]; if(!s) return;
  var inner=document.getElementById('pres-inner');
  var area=document.getElementById('pres-area');
  var baseW=Math.min((area.clientWidth||1200)*0.90, 1100);
  var baseH=Math.round(baseW*9/16);
  inner.style.width=baseW+'px';
  inner.style.height=baseH+'px';
  var isDark=SM==='dark'||s.isCover;
  var bg=s.isCover?'background:linear-gradient(135deg,#0f2027,#203a43,#2c5364)':isDark?'background:linear-gradient(135deg,#1a1a2e,#16213e)':'background:#fff';
  var tc=isDark?'color:#e8f4fd':'color:#1a1a2e';
  var bc=isDark?'color:#b8d4f0':'color:#333';
  var dot=s.isCover?'display:none':'background:#4f8ef7';
  var tMin=Math.round(18*fontScale), tMax=Math.round(40*fontScale), tVw=(3.2*fontScale).toFixed(1);
  var bMin=Math.round(10*fontScale), bMax=Math.round(20*fontScale), bVw=(1.6*fontScale).toFixed(1);
  var titleHtml=s.titleHtml!=null?s.titleHtml:escH(s.title);
  var bulletsHtml=s.bulletsHtml||[];
  var h='<div style="position:absolute;inset:0;padding:5% 6%;display:flex;flex-direction:column;'+bg+(s.isCover?';align-items:center;justify-content:center;text-align:center':'')+'">';
  h+='<div style="position:absolute;left:0;top:0;bottom:0;width:0.6%;background:#4f8ef7;'+(s.isCover?'display:none':'')+'" ></div>';
  h+='<div style="font-size:clamp('+tMin+'px,'+tVw+'vw,'+tMax+'px);font-weight:700;line-height:1.2;margin-bottom:4%;'+tc+(s.isCover?';border-bottom:2px solid rgba(79,142,247,0.5);padding-bottom:18px;margin-bottom:14px':'')+'">'+titleHtml+'</div>';
  h+='<div style="flex:1">';
  if(bulletsHtml.length>0){
    bulletsHtml.forEach(function(html){
      h+='<div style="display:flex;gap:10px;margin-bottom:1.8%;align-items:flex-start"><div style="width:7px;height:7px;border-radius:50%;flex-shrink:0;margin-top:0.45em;'+dot+'"></div>';
      h+='<div style="font-size:clamp('+bMin+'px,'+bVw+'vw,'+bMax+'px);line-height:1.5;'+bc+'">'+html+'</div></div>';
    });
  } else {
    (s.bullets||[]).forEach(function(b){
      h+='<div style="display:flex;gap:10px;margin-bottom:1.8%;align-items:flex-start"><div style="width:7px;height:7px;border-radius:50%;flex-shrink:0;margin-top:0.45em;'+dot+'"></div>';
      h+='<div style="font-size:clamp('+bMin+'px,'+bVw+'vw,'+bMax+'px);line-height:1.5;'+bc+'">'+escH(b)+'</div></div>';
    });
  }
  h+='</div></div>';
  if(s.imageUrl) h+='<img style="position:absolute;right:0;top:0;bottom:0;width:38%;object-fit:cover" src="'+s.imageUrl+'" alt=""/>';
  inner.innerHTML=h;
  document.getElementById('counter').textContent=(idx+1)+'/'+SL.length;
  var nd=document.getElementById('notes');
  nd.textContent=SC[idx]||s.notes||'';
  if(nv) nd.classList.add('show'); else nd.classList.remove('show');
  applyZoom();
  var fv=document.getElementById('fv');
  if(fv) fv.textContent=Math.round(fontScale*100)+'%';
}
function nav(d){ idx=Math.max(0,Math.min(SL.length-1,idx+d)); render(); }
...
document.addEventListener('keydown',function(e){
  ...
  else if(e.ctrlKey&&e.key==='ArrowRight'){e.preventDefault();nav(1);}
  else if(e.ctrlKey&&e.key==='ArrowLeft'){e.preventDefault();nav(-1);}
  else if(e.key==='Escape') window.close();
  else if(e.key==='n'||e.key==='N') toggleNotes();
  else if(e.ctrlKey&&e.key==='9'){e.preventDefault();chZoom(-10);}
  else if(e.ctrlKey&&e.key==='0'){e.preventDefault();chZoom(+10);}
  else if(!e.ctrlKey&&(e.key==='+'||e.key==='=')) chZoom(+10);
  else if(!e.ctrlKey&&e.key==='-') chZoom(-10);
  else if(e.ctrlKey&&e.altKey&&(e.key==='+'||e.key==='=')){e.preventDefault();chFontScale(0.1);}
  else if(e.ctrlKey&&e.altKey&&e.key==='-'){e.preventDefault();chFontScale(-0.1);}
  else if(e.key==='f'||e.key==='F') fitZoom();
});
window.addEventListener('resize',function(){ render(); setTimeout(fitZoom,30); });
render();
  ...
  if (!rawText) {
    content.innerHTML = `
      <div class="upload-zone" id="upload-drop-zone" onclick="document.getElementById('file-input').click()"
        ondragover="handleDragOver(event)" ondrop="handleDrop(event)" ondragleave="handleDragLeave()">
        <input type="file" id="file-input" style="display:none" accept=".pdf,.docx,.txt" onchange="handleFileUpload(event)"/>
        <span class="upload-icon">📄</span><h3>논문 업로드</h3><p>PDF, DOCX, TXT · 드래그 앤 드롭</p>
      </div>
      <div class="text-input-zone">
        <label class="label" style="margin-top:12px">또는 텍스트 직접 붙여넣기</label>
        <textarea id="text-paste-input" placeholder="논문 본문을 여기에 붙여넣으세요..." rows="6"></textarea>
        <button class="btn btn-ghost w-full mt-2" style="justify-content:center" onclick="loadFromTextInput()">✅ 텍스트 로드</button>
      </div>`;
    return;
  }
  const customVal = document.getElementById('custom-instruction-val')?.value || '';
  const slideCountVal = document.getElementById('slide-count-val')?.value || '12';
  let displayContent = '';
  if (leftTab === 'summary') {
    displayContent = `<div class="translate-row">
      ...
      ${summaryText ? '<button class="btn btn-ghost btn-xs" onclick="openSummaryWindow()">🔲 전체 보기</button>' : ''}
    ...
    displayContent = `<div class="translate-row">
      ...
      ${rawText.length > 5000 ? '<button class="btn btn-ghost btn-xs" onclick="openFullTextWindow()">🔲 전체 보기</button>' : ''}
    </div><div style="font-size:10px;line-height:1.6;color:var(--text2);white-space:pre-wrap;font-family:var(--font-mono);max-height:340px;overflow-y:auto;margin-top:6px">${escapeHtml(rawText.substring(0, 5000))}${rawText.length > 5000 ? '\n\n[... 더 있음 — 위의 🔲 전체 보기 버튼을 클릭하세요]' : ''}</div>`;
    ...
    if (!presentationScript.length) {
      ...
      displayContent = `<div class="translate-row"><button class="btn btn-ghost btn-xs" onclick="saveContent('script')">💾 원고 저장</button></div>${presentationScript.map((s, i) => `<div class="script-slide-section"><div class="script-slide-label">슬라이드 ${i + 1}</div><div class="script-slide-title">${escapeHtml(slides[i]?.title || '')}</div><div class="script-slide-content">${escapeHtml(s)}</div></div>`).join('')}`;
    }
  }
  const _isPdf = fileName.toLowerCase().endsWith('.pdf');
  content.innerHTML = `
    ...
    <div style="margin-bottom:8px">
      <label class="label">슬라이드 모드</label>
      <div class="action-grid">
        <button class="action-card mode-academic ${slideStyle === 'light' ? 'active' : ''}" onclick="setSlideStyle('light')"><span class="action-card-icon">☀️</span>라이트모드</button>
        <button class="action-card mode-presentation ${slideStyle === 'dark' ? 'active' : ''}" onclick="setSlideStyle('dark')"><span class="action-card-icon">🌙</span>다크모드</button>
      </div>
    </div>
    <div style="margin-bottom:8px">
      <label class="label">문체 설정</label>
      <select class="control" id="writing-style-val" onchange="setWritingStyle(this.value)" style="font-size:11px">
        ...
        <option value="polite" ${writingStyle === 'polite' ? 'selected' : ''}>일반체 (존댓말)</option>
      </select>
    </div>
    <div style="display:flex;gap:6px;align-items:center;margin-bottom:8px">
      <label class="label" style="margin:0;white-space:nowrap">슬라이드 수</label>
      <input type="number" class="control" id="slide-count-val" value="${slideCountVal}" min="5" max="30" style="width:64px;text-align:center"/>
      <label style="display:flex;align-items:center;gap:4px;font-size:11px;color:var(--text2);cursor:pointer;white-space:nowrap;margin-left:4px">
        <input type="checkbox" id="include-cover" checked style="accent-color:var(--accent)"/> 표지 포함
      </label>
    </div>
    <div class="action-grid">
      <button class="action-card" onclick="askThenSummary('full')"><span class="action-card-icon">📖</span>전체 요약</button>
      <button class="action-card" onclick="askThenSummary('slides')"><span class="action-card-icon">🗂</span>슬라이드 생성</button>
    </div>
    <label class="label">커스텀 지시사항</label>
    <textarea class="control" id="custom-instruction-val" rows="2" placeholder="예: 통계 방법론 집중, 영어로 출력...">${customVal}</textarea>
...
function setSlideStyle(s) { slideStyle = s; renderLeftPanel(); if (slides.length) renderSlides(); }
function setWritingStyle(s) {
  writingStyle = s;
  showToast(`✅ 문체: ${s === 'academic-da' ? '학술체 (~이다)' : s === 'academic-im' ? '학술체 (~임, ~함)' : '일반체 (존댓말)'}으로 설정됨 — 슬라이드 생성 시 적용됩니다`);
}
...
function openFullTextWindow() {
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>원문 전체 — ${escapeHtml(fileName)}</title><style>body{margin:0;padding:20px 24px;background:#0c0e13;color:#b0bac8;font-family:'JetBrains Mono',monospace;font-size:12px;line-height:1.7;white-space:pre-wrap;word-wrap:break-word}h2{font-family:sans-serif;font-size:15px;color:#4f8ef7;border-bottom:1px solid #252a37;padding-bottom:10px;margin-bottom:16px}.toolbar{display:flex;align-items:center;gap:8px;margin-bottom:16px}.copy-btn{background:#4f8ef7;color:#fff;border:none;border-radius:6px;padding:6px 14px;font-size:12px;cursor:pointer}</style></head><body><div class="toolbar"><h2 style="margin:0;flex:1">📄 ${escapeHtml(fileName)} — 원문 전체 (${(rawText.length / 1000).toFixed(1)}k자)</h2><button class="copy-btn" onclick="navigator.clipboard.writeText(document.getElementById('txt').textContent).then(()=>alert('복사됨'))">📋 복사</button></div><div id="txt">${escapeHtml(rawText)}</div></body></html>`);
  win.document.close();
}
...
function openSummaryWindow() {
  ...
  if (!win) { showToast('⚠️ 팝업이 차단되었습니다. 팝업 허용 후 다시 시도하세요.'); return; }
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>요약 전체 — ${escapeHtml(fileName)}</title>
<style>
* { box-sizing:border-box; margin:0; padding:0; }
body { background:#0c0e13; color:#e8ecf4; font-family:'Noto Sans KR',sans-serif; font-size:13px; line-height:1.8; }
.toolbar { background:#13161d; border-bottom:1px solid #1e2332; padding:10px 20px; display:flex; align-items:center; gap:10px; position:sticky; top:0; }
h2 { font-size:14px; color:#4f8ef7; flex:1; }
.copy-btn { background:#4f8ef7; color:#fff; border:none; border-radius:6px; padding:6px 14px; font-size:12px; cursor:pointer; }
.content { padding:24px 28px; white-space:pre-wrap; word-break:break-word; max-width:800px; margin:0 auto; color:#cdd5e0; }
</style></head><body>
<div class="toolbar">
  <h2>📋 요약 전체 — ${escapeHtml(fileName)}</h2>
  <button class="copy-btn" onclick="navigator.clipboard.writeText(document.getElementById('sc').textContent).then(()=>alert('복사됨'))">📋 복사</button>
  <button class="copy-btn" style="background:#1a1e28;border:1px solid #2e3447;color:#b0bac8" onclick="window.close()">닫기</button>
</div>
<div class="content" id="sc">${escapeHtml(summaryText)}</div>
</body></html>`);
  win.document.close();
}
...
function afterSlidesCreated() {
  ...
  if (cnt) { cnt.textContent = slides.length + ' 슬라이드'; cnt.style.display = 'inline'; }
  ['export-btn', 'visualize-btn', 'save-session-btn', 'fetch-sources-btn', 'gen-script-btn', 'present-btn', 'present-from-current-btn']
...
function selectSlide(index, fromThumb) {
  ...
  document.querySelectorAll('.slide-layer-selectable').forEach(function (el) { el.classList.remove('layer-selected'); });
  document.querySelectorAll('.layer-toolbar').forEach(function (el) { el.style.display = 'none'; });
  selectedLayer = null;
  updateDesignPanel();
  const el = document.getElementById(`sw-${index}`);
  if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' });

  // ✅ 하단 썸네일에서 선택 시 MD에디터 탭으로 전환
  if (fromThumb && rightTab !== 'mdeditor') {
    switchRightTab('mdeditor');
  }

  // MD 에디터 자동 로드 + 페이지 표시 업데이트
  try {
    if (typeof window.mdLoadFromSlide === 'function') window.mdLoadFromSlide();
    if (typeof window.mdUpdatePageIndicators === 'function') window.mdUpdatePageIndicators();
    if (typeof window.updateWhitespaceBadges === 'function') window.updateWhitespaceBadges();
  } catch (e) { }
}

// ── 레이어 순서 조절 (위로/아래로) ─────────────────────
let selectedLayer = null; // { slideIndex, layer: 'text'|'image' }

function selectSlideLayer(slideIndex, layer, event) {
  if (!event || !event.target) return;
  if (event.target.closest('button, input, textarea, .youtube-resize-handle, .youtube-move-handle, .bullet-position-handle, .bullet-resize-handle, .slide-image-remove, .slide-extra-position-handle, .slide-extra-move-btn, .slide-extra-resize-handle, .bullet-rendered, .bullet-content')) return;
  event.stopPropagation();
  selectedLayer = { slideIndex, layer };
  document.querySelectorAll('.slide-layer-selectable').forEach(function (el) { el.classList.remove('layer-selected'); });
  document.querySelectorAll('.layer-toolbar').forEach(function (el) { el.style.display = 'none'; });
  var wrap = document.getElementById('sw-' + slideIndex);
  if (wrap) {
    var sel = wrap.querySelector('[data-layer="' + layer + '"]');
    if (sel) sel.classList.add('layer-selected');
    var bar = document.getElementById('layer-toolbar-' + slideIndex);
    if (bar) bar.style.display = 'flex';
  }
}

function moveLayerUp(slideIndex) {
  if (!slides[slideIndex]) return;
  var slide = slides[slideIndex];
  var order = slide.layerOrder || ['text', 'image'];
  var idx = order.indexOf(selectedLayer && selectedLayer.slideIndex === slideIndex ? selectedLayer.layer : order[0]);
  if (idx < 0 || idx >= order.length - 1) return;
  var next = order[idx + 1];
  order[idx + 1] = order[idx];
  order[idx] = next;
  slide.layerOrder = order;
  applyLayerOrderToDom(slideIndex);
}

function moveLayerDown(slideIndex) {
  if (!slides[slideIndex]) return;
  var slide = slides[slideIndex];
  var order = slide.layerOrder || ['text', 'image'];
  var idx = order.indexOf(selectedLayer && selectedLayer.slideIndex === slideIndex ? selectedLayer.layer : order[0]);
...
function applyLayerOrderToDom(slideIndex) {
  var slide = slides[slideIndex];
  if (!slide) return;
  var order = slide.layerOrder || ['text', 'image'];
  var wrap = document.getElementById('sw-' + slideIndex);
  if (!wrap) return;
  var textEl = wrap.querySelector('.slide-inner[data-layer="text"]');
  var imageEl = wrap.querySelector('.slide-image-pane[data-layer="image"]');
  if (textEl) textEl.style.zIndex = order.indexOf('text') >= 0 ? order.indexOf('text') : 0;
  if (imageEl) imageEl.style.zIndex = order.indexOf('image') >= 0 ? order.indexOf('image') : 1;
}
...
function slideUndo() {
  ...
  try {
    ...
    if (countEl) countEl.textContent = slides.length + ' 슬라이드';
    showToast('↩ 실행 취소');
...
function openImageModal2(slideIdx) {
  ...
  openImageModal(idx);
  // After image is applied, it goes to imageUrl2 instead of imageUrl
...
function askThenVisualizeAll() {
  ...
  const toGen = slides.filter(s => s.visPrompt && !s.imageUrl).length;
...
async function startVisualizeAll() {
  ...
  const targets = slides.filter(s => s.visPrompt && (overwrite ? true : !s.imageUrl));
  ...
  _bgJobEnd(_bgJobCancelled
    ? `⏹ 중단됨 — ${_bgJob.done}/${_bgJob.total}개 생성`
...
async function refineImage() {
  ...
  showLoading('이미지 수정 중...', instruction, 40, true);
  const newImg = await refineImageAPI(slide.imageUrl, instruction);
  if (newImg) {
    if (typeof pushSlideUndoState === 'function') pushSlideUndoState();
...
function renderGallery() {
  ...
  if (grid) grid.innerHTML = imaged.map(s => {
    ...
    return `<div class="gallery-item">
      <img src="${s.imageUrl}" alt="" onclick="selectSlide(${idx});switchRightTab('design')" style="cursor:pointer"/>
...
function renderPresSlide() {
  ...
  const bulletColor = isDark ? 'color:#b8d4f0' : 'color:#333';
  const dotBg = isCover ? 'display:none' : (isDark ? 'background:#4f8ef7' : 'background:#4f8ef7');

  const fs = _slideFontScale / 100;
  ...
  const innerHTML = `
    <div style="position:absolute;inset:0;padding:5% 6%;display:flex;flex-direction:column;${bgCss};${isCover ? 'align-items:center;justify-content:center;text-align:center' : ''}">
      <div style="position:absolute;left:0;top:0;bottom:0;width:0.6%;background:#4f8ef7;${isCover ? 'display:none' : ''}"></div>
      <div style="font-size:clamp(${tMin}px,${tVw}vw,${tMax}px);font-weight:700;line-height:1.2;margin-bottom:4%;font-family:var(--font-head);${titleColor}${isCover ? ';border-bottom:2px solid rgba(79,142,247,0.5);padding-bottom:20px;margin-bottom:16px' : ''}">${markdownToHtml(slide.title)}</div>
      <div style="flex:1">
        ${slide.bullets.map(b => `<div style="display:flex;gap:12px;margin-bottom:2%;align-items:flex-start">
          <div style="width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:0.5em;${dotBg}"></div>
          <div style="font-size:clamp(${bMin}px,${bVw}vw,${bMax}px);line-height:1.5;${bulletColor}">${markdownToHtml(b)}</div>
        </div>`).join('')}
      </div>
    </div>
    ${slide.imageUrl ? `<img style="position:absolute;right:0;top:0;bottom:0;width:40%;object-fit:cover;" src="${slide.imageUrl}" alt=""/>` : ''}
  `;
  ...
  if (presFontEl) presFontEl.textContent = _slideFontScale + '%';
...
function clearAll() {
  ...
  ['export-btn', 'visualize-btn', 'fetch-sources-btn', 'save-session-btn', 'gen-script-btn', 'present-btn']
...
// ── Open image modal ─────────────────────────────────────
function openImageModal(slideIdx) {
  ...
  // ✅ 슬라이드에 이미지가 이미 있으면 바로 크롭 편집 모드로 열기
  const existingImg = slides[_targetSlideForImage]?.imageUrl;
...
function applyImageToSlide() {
  ...
  if (window._applyAsSecondImage) {
    slides[_targetSlideForImage].imageUrl2 = dataURL;
    window._applyAsSecondImage = false;
    showToast('✅ 두 번째 이미지 삽입 완료');
  } else {
    slides[_targetSlideForImage].imageUrl = dataURL;
    showToast('✅ 이미지 삽입 완료');
  }
  renderSlides(); renderThumbs(); renderGallery();
...
function openImageFullscreen(dataURL) {
  const w = window.open('', '_blank');
  w.document.write(`<!DOCTYPE html><html><head><title>이미지 미리보기</title>
  <style>body{margin:0;background:#111;display:flex;align-items:center;justify-content:center;min-height:100vh}
  img{max-width:98vw;max-height:98vh;object-fit:contain;border-radius:8px;box-shadow:0 8px 40px rgba(0,0,0,0.8)}
  </style></head><body><img src="${dataURL}"/></body></html>`);
}

// ── Patched generateSingleImage — shows result below, records history ──
async function generateSingleImage(idx) {
  ...
  showLoading(`슬라이드 ${i + 1} 이미지 생성 중...`, p, 30, true);

  // Show inline generating indicator
  const resultArea = document.getElementById('ai-img-inline-result');
  if (resultArea) { resultArea.innerHTML = `<div style="text-align:center;padding:12px;color:var(--text2);font-size:11px">🎨 생성 중... 잠시만 기다려주세요</div>`; resultArea.style.display = 'block'; }

  const img = await generateImage(p);
  hideLoading();

  if (img) {
    // ✅ Save to history ONLY — do NOT auto-apply to slide
    addToAiImgHistory(p, img, i);

    // Show result card with "apply" button
    if (resultArea) {
...
function renderSlides() {
  ...
  canvas.innerHTML = slides.map((slide, index) => {
    ...
    const layerOrder = slide.layerOrder || ['text', 'image'];
    ...
    <div class="slide-wrapper ${index === activeSlideIndex ? 'active' : ''}" id="sw-${index}" onclick="selectSlide(${index})">
      ...
      <div class="slide-card ${coverStyle}">
        <div class="slide-inner slide-layer-selectable${slide.imageUrl ? ' has-image' : ''}" data-slide-index="${index}" data-layer="text" style="${layerStyleText}" onclick="event.target.closest('.slide-layer-selectable')&&selectSlideLayer(${index},'text',event)">
          <textarea class="slide-title-input" rows="2"
        ...
        ${slide.imageUrl ? `<div class="slide-image-pane slide-layer-selectable${slide.imageUrl2 ? ' dual-image' : ''}" data-slide-index="${index}" data-layer="image" style="${layerStyleImage}" onclick="event.target.closest('.slide-layer-selectable')&&selectSlideLayer(${index},'image',event)">
        ...
        <div class="layer-toolbar" id="layer-toolbar-${index}" data-slide-index="${index}" style="display:none">
          <span class="layer-toolbar-label">레이어</span>
          <button type="button" class="layer-toolbar-btn" onclick="moveLayerUp(${index});event.stopPropagation()" title="한 칸 앞으로">↑ 위로</button>
          <button type="button" class="layer-toolbar-btn" onclick="moveLayerDown(${index});event.stopPropagation()" title="한 칸 뒤로">↓ 아래로</button>
        </div>
...
(function () {
  ...
  document.addEventListener('contextmenu', function (e) {
    if (e.target && (e.target.closest('.youtube-resize-handle') || e.target.closest('.youtube-move-handle') || e.target.closest('.bullet-resize-handle') || e.target.closest('.bullet-position-handle'))) e.preventDefault();
  }, true);
})();
...
// ── 본문(불릿) 영역 좌우/상하 크기 조절 (4방향 리사이즈 핸들) ─────────────────────
(function () {
...
function viewFull(){if(!_url)return;var w=window.open('','_blank');w.document.write('<body style="margin:0;background:#111"><img src="'+_url+'" style="max-width:100vw;max-height:100vh;display:block;margin:auto"></body>');}
<\/script></body></html>`;
  w.document.write(html);
  w.document.close();
}
...
// =====================================================
document.addEventListener('DOMContentLoaded', () => {
  try { switchRightTab('mdeditor'); } catch (e) { }
});
